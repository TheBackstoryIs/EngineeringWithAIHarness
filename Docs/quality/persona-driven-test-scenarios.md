...
New records use versioned intent-content fingerprints. Only EWAI's workflow frontmatter (`status`, `delivery_status`, `current_phase`, `delivery_state_path` and `updated_at`) is excluded. The complete intent body and all other metadata still participate, including content beyond the displayed excerpts. Plan, Test Plan, standards and impact sources retain their existing whole-file fingerprints. Moving to another delivery phase does not, by itself, change the agreed test expectations.

Preparation requires at least one recognised intent source: captured text under `Problem`, `Desired outcome` or `Constraints`, or a listed journey or acceptance criterion. Planning documents alone cannot anchor an intent fingerprint. Missing intent sources fail closed, including for historical packs. Non-workflow metadata must use plain mappings, arrays and JSON scalar values; explicitly typed YAML dates, sets, ordered maps, binary values, non-finite numbers and cyclic aliases are rejected before a content fingerprint is accepted. Use quoted date strings and ordinary lists where appropriate. Existing evidence is preserved if current sources become unsupported; correct the source through named review, not by editing the evidence pair.

Older unversioned packs keep conservative whole-file checking. Before their sources change, repeating the original `record` command with the unchanged candidate and the same named reviewer adds `test-scenarios.source-baseline.json`. This write-once compatibility receipt binds the still-current sources to the original record. Neither accepted JSON nor Markdown is rewritten. Preparation and status do not create the receipt. Keep it with the evidence pair: a missing receipt restores legacy checking, and an invalid or mismatched receipt fails closed. Already-stale legacy evidence cannot be silently upgraded, and this compatibility path does not accept revised expectations or replace the named review required for genuine source changes.
